...
class WishlistIntegrationTest extends IntegrationTestSupport {
  ...
  void 위시리스트_생성_메모수정_조회_삭제_전체_흐름() throws Exception {
    CreateWishlistRequest createRequest =
        new CreateWishlistRequest("협재해수욕장", "관광지", "제주 한림읍", 33.39, 126.24, "친구 추천");
    ...
    String getAllAfterCreateJson =
        mockMvc
            ...
            .getResponse()
            .getContentAsString();
    List<WishlistResponse> afterCreate =
        objectMapper.readValue(
            getAllAfterCreateJson,
            objectMapper
                .getTypeFactory()
    ...
    String updateResponseJson =
        mockMvc
            .perform(
                patch("/wishlist/{id}/memo", created.id())
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc.perform(delete("/wishlist/{id}", created.id())).andExpect(status().isNoContent());

    String getAllAfterDeleteJson =
        mockMvc
            .perform(get("/wishlist"))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse()
            .getContentAsString();
    List<WishlistResponse> afterDelete =
        objectMapper.readValue(
            getAllAfterDeleteJson,
            objectMapper
                .getTypeFactory()
                .constructCollectionType(List.class, WishlistResponse.class));
