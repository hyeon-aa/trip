...
class JejuPlaceRepositoryIntegrationTest extends IntegrationTestSupport {
  ...
  @Autowired private JejuPlaceRepository jejuPlaceRepository;
  ...
  void 지역_필터로_동부만_걸러진다() {
    List<JejuPlace> result =
        jejuPlaceRepository.findSimilarPlacesWithFilter(vectorLiteral(1.0, 0.0), "동부", "{}", 10);

    assertThat(result)
        .extracting(JejuPlace::getId)
  ...
  void 카테고리_필터로_관광지만_걸러진다() {
    List<JejuPlace> result =
        jejuPlaceRepository.findSimilarPlacesWithFilter(vectorLiteral(1.0, 0.0), null, "{관광지}", 10);
  ...
  void 지역과_카테고리_필터를_동시에_걸_수_있다() {
    List<JejuPlace> result =
        jejuPlaceRepository.findSimilarPlacesWithFilter(vectorLiteral(1.0, 0.0), "동부", "{관광지}", 10);
  ...
  void 필터가_없으면_코사인_거리가_가까운_순서로_정렬된다() {
    List<JejuPlace> result =
        jejuPlaceRepository.findSimilarPlacesWithFilter(vectorLiteral(1.0, 0.0), null, "{}", 10);
