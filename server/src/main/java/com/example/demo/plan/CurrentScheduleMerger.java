...
public class CurrentScheduleMerger {
    ...
    public CurrentScheduleMerger(JejuPlaceRepository jejuPlaceRepository, WishlistRepository wishlistRepository) {
    ...
    public String mergeAndBuildSection(
        ...
        for (ScheduleDto.DayDto day : currentSchedule.days()) {
            ...
            if (!entries.isEmpty()) {
                section.append(day.day()).append("일차: ")
    ...
    private boolean resolveOrFetch(
        ...
        if (id.charAt(0) == 'p') {
            return jejuPlaceRepository.findById(dbId)
                .map(p -> { placeIdMap.put(id, p); return true; })
                .orElse(false);
        }
        if (id.charAt(0) == 'w') {
            return wishlistRepository.findById(dbId)
                .map(w -> { wishlistIdMap.put(id, w); return true; })
                .orElse(false);
        }
        return false;
    }
