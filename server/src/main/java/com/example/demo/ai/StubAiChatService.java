...
public class StubAiChatService implements AiChatService {
    ...
    private String buildScheduleResponse(List<ChatMessageDto> messages) {
        ...
        List<String> day2 = idList.size() > 3
            ? idList.subList(3, idList.size())
            : List.of();

        String day1Places = day1.stream()
            .map(id -> "{\"id\":\"" + id + "\",\"reason\":\"E2E 테스트용 추천 이유\",\"recommendedTime\":\"10:00~11:00\"}")
            .collect(Collectors.joining(","));
        String day2Places = day2.stream()
            .map(id -> "{\"id\":\"" + id + "\",\"reason\":\"E2E 테스트용 추천 이유\",\"recommendedTime\":\"10:00~11:00\"}")
            .collect(Collectors.joining(","));

        String daysJson = day2Places.isEmpty()
            ? "[{\"day\":1,\"places\":[" + day1Places + "]}]"
            : "[{\"day\":1,\"places\":[" + day1Places + "]},{\"day\":2,\"places\":[" + day2Places + "]}]";

        return "{\"type\":\"schedule\",\"message\":\"E2E 테스트용 일정이에요!\",\"schedule\":{\"days\":" + daysJson + "}}";
    }
