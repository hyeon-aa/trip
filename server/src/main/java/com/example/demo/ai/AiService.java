...
public class AiService implements AiChatService {
  ...
  public String chat(List<ChatMessageDto> messages) {
    ...
    Map<String, Object> body = Map.of("model", "llama-3.3-70b-versatile", "messages", messages);
  ...
  public String chatWithGemini(List<ChatMessageDto> messages, String responseSchema) {
    ...
    Prompt prompt =
        responseSchema == null
            ...
            : new Prompt(
                chatMessages,
                GoogleGenAiChatOptions.builder().outputSchema(responseSchema).build());
    int attempts = 0;
    while (true) {
      try {
        ...
        System.out.println(
            "[chatWithGemini] Gemini 응답 소요시간: " + (System.currentTimeMillis() - start) + "ms");
        return result;
