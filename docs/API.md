...
| Method | Path | Body | Response |
|---|---|---|---|
| POST | `/plan/chat` | `PlanChatRequest { message: string, history: ChatMessageDto[] }` | `text/event-stream` (SSE), 단일 이벤트로 최종 JSON 전송 |
...
address, lat, lng, description, embedding(text, pgvector 캐스팅용)`.
