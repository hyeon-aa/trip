...
- **CORS**: `CorsConfig`에서 `http://localhost:3000`만 허용 (`GET/POST/PUT/PATCH/DELETE`).
...
|---|---|---|---|
| POST | `/wishlist` | `CreateWishlistRequest { name, category, address, lat, lng, memo }` | `WishlistResponse` |
| GET | `/wishlist` | - | `WishlistResponse[]` |
| DELETE | `/wishlist/{id}` | - | 204 |
| PATCH | `/wishlist/{id}/memo` | `UpdateWishlistMemoRequest { memo }` | `WishlistResponse` |

`WishlistResponse { id, name, category, address, lat, lng, memo }` — `memo`는
왜 저장했는지(추천받아서/SNS에서 봐서 등) 남기는 nullable 텍스트. 저장 시점에는 비워둘 수
있고, `PATCH /wishlist/{id}/memo`로 언제든 수정한다.
...
`Wishlist` (`wishlist` 테이블): `id, name, lat, lng, category, kakaoPlaceId, address, memo`.
