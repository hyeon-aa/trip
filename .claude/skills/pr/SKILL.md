...
- **이 작업이 특정 GitHub 이슈에서 시작됐다면 본문에 `Closes #N`을 반드시
  넣는다**(Summary 맨 위나 맨 아래, 별도 줄로). 그냥 본문 텍스트에 "이슈
  #50" 식으로 언급만 하면 GitHub이 자동으로 안 닫아준다 — `Closes`/`Fixes`/
  `Resolves` 키워드가 있어야 머지 시 그 이슈가 자동으로 닫힌다(2026-07-28,
  이슈 #50이 PR #55 머지 후에도 안 닫혀서 뒤늦게 수동으로 닫은 적 있음).
  이슈 번호가 헷갈리면 브랜치를 만들 때 참고한 이슈(`/issue` 4단계) 또는
  대화 맥락에서 찾는다.
...
**예외 — 코드 변경이 없는 PR은 리뷰를 생략한다.** `git diff origin/main...HEAD
--stat`로 바뀐 파일이 전부 `.md`/`docs/**`/`.claude/**` 같은 문서·스킬
파일뿐이고 `server/`·`client/` 아래 실제 코드가 하나도 없으면, 버그가 날
표면 자체가 없으므로 `/code-review`를 돌리지 않고 CI 통과만 확인한다(단,
CI가 실제로 그 문서 변경으로 실패하는 경우는 당연히 원인을 봐야 한다).
바뀐 파일에 코드가 하나라도 섞여 있으면 평소대로 무조건 리뷰를 돌린다.
...
이 PR이 특정 이슈에서 시작됐는데 본문에 `Closes #N`을 못 넣었다면(예: 기존
PR이라 이미 생성돼 있었던 경우), 머지 직후 `gh issue close <N>`으로 수동으로
닫는다.
